...
const Button = ({ status, date }) => {

    return (
        status == "scheduled" ? <button className={`flex font-semibold h-[40px] w-[110px] rounded justify-center items-center`}>
            {/* Date as prop in real world */}
            <Countdown targetTime={date} />
        </button>
            : status == "join" ?
                <button className="bg-blue-500 hover:bg-blue-700 text-white font-semibold px-2 py-2 border border-gray-400 rounded shadow flex gap-2">
                    Join now
                ...
                : status == "book" ?
                    <button className="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-[#FFFFFF] rounded shadow">
                        Book now
                    </button>
